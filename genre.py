...
# songs_df = songs_df.sample(50000)
# print(len(songs_df))
# print(songs_df.head())
...
for genre in genres:
    ...
    if genre == 'Rock': subset = subset[:19000]
...
plt.title('Correlation Matrix - 5 genres')
plt.show()
...
print("Hip-Hop:", probabilities[0][1])
print("Metal:", probabilities[0][2])
print("Pop:", probabilities[0][3])
print("Rock:", probabilities[0][4])
...
# map_songs = songs_df.loc[:999]
# probabilities = text_clf.predict_proba(map_songs['lyrics'])  # Predict for the lyrics provided

# map_songs['Country'], map_songs['Hip-Hop'], map_songs['Metal'], map_songs['Pop'], map_songs['Rock'] = \
#    ['', '', '', '', '']
...
#    map_songs.loc[row.Index, 'Hip-Hop'] = probabilities[n][1]
#    map_songs.loc[row.Index, 'Metal'] = probabilities[n][2]
#    map_songs.loc[row.Index, 'Pop'] = probabilities[n][3]
#    map_songs.loc[row.Index, 'Rock'] = probabilities[n][4]
...
map_songs = pd.read_csv('./datasets/final_small.csv')  # 1000 entries with all the genres, lyrics processed and added probabilities
RANDOM_STATE = 42
